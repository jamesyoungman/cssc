...
check_pfile() {
    ...
    makep "$pbody"
    # shellcheck disable=SC2086 # options is deliberately unquoted
...
if ! makep "1.1 1.2 james 16/02/28 10:59:47${newline}"
then
...
# stderr message)
check_pfile  --nostderr_regex S4 '1.1 1.2 james 16/02/28 10:59:47'"${newline}" 0 IGNORE ""

# Verify that it's OK for the date in the pfile to be in the past.
# We later rely on this fact in test S9.
check_pfile --nostdout_regex S4a '1.1 1.2 james 15/02/28 10:59:47'"${newline}" 0 '1.1 1.2 james 15/02/28 10:59:47'"${newline}" IGNORE
...
check_pfile S6 'x.y 1.2 james 16/02/28 10:59:47'"${newline}" 1 IGNORE "SID"
check_pfile S7 '1.1 x.y james 16/02/28 10:59:47'"${newline}" 1 IGNORE "SID"

# The username is basically free-format, just make sure it's not missing.
check_pfile S8 '1.1 1.2 16/02/28 10:59:47'"${newline}" 1 IGNORE "Expected 5-7 args"

# Invalid date (there are not that many days in February 2015)
check_pfile S9 '1.1 1.2 james 15/02/29 10:59:47'"${newline}" 1 IGNORE "Invalid date"

# Valid time, 24 clock
check_pfile --nostdout_regex --nostderr_regex S10 '1.1 1.2 james 16/02/28 23:59:47'"${newline}" 0 \
	    '1.1 1.2 james 16/02/28 23:59:47\n' ""

# Invalid times
check_pfile S11 '1.1 1.2 james 16/02/28 25:59:47'"${newline}" 1 IGNORE "Invalid date/time"
check_pfile S12 '1.1 1.2 james 16/02/28 23:69:47'"${newline}" 1 IGNORE "Invalid date/time"
check_pfile S13 '1.1 1.2 james 16/02/28 23:49:67'"${newline}" 1 IGNORE "Invalid date/time"

# Make sure we allow a leap second.
check_pfile  --nostdout_regex --nostderr_regex S14 '1.1 1.2 james 16/02/28 23:59:60'"${newline}" 0 \
...
check_pfile  --nostdout_regex --nostderr_regex S15 \
	     '1.3 1.4 james 16/02/28 11:55:42 -i1.1-1.2 -x1.3'"${newline}" 0 \
	     '1.3 1.4 james 16/02/28 11:55:42\n' ""
# Just included deltas
check_pfile   --nostdout_regex  --nostderr_regex S16 '1.3 1.4 james 16/02/28 11:55:42 -i1.1-1.2'"${newline}" 0 IGNORE ""
# Just excluded deltas
check_pfile  --nostderr_regex S17 '1.3 1.4 james 16/02/28 11:55:42 -x1.1-1.2'"${newline}" 0 IGNORE ""
# Included and excluded deltas, but in the other order
check_pfile  --nostderr_regex S18 '1.3 1.4 james 16/02/28 11:55:42 -x1.1-1.2 -i1.3'"${newline}" 0 IGNORE ""
