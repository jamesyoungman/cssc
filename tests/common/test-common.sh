...
copy() {
    ...
    copy_from="${2}"
    copy_to="${3}"
    # In case ${to} is a symlink to something we don't want to modify,
    # ensure that cp will create ${to} instead of overwriting any
    # existing contents.
    rm -f -- "${copy_to}"
...
set_and_maybe_print_step_label_with_dots() {
    set_test_script_and_label "$@"
    "${silent:-false}" || echo_nonl "${test_script}:${label}"...
}
...
do_output_bin () {
    set_test_script_and_label "$@"
    ../../testutils/do_cmd --test_name="${test_script}" --stdout_is_file "$@"
}
